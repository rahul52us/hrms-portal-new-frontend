...
export default function LeaveBalancesPanel({ companyId, borderColor, muted }: { companyId: string; borderColor: string; muted: string }) {
  ...
  return (
    <Stack spacing={5}>
      ...
      {!employeeId ? <Box py={12} textAlign="center" borderWidth="1px" borderStyle="dashed" borderColor={borderColor} borderRadius="md"><Text color={muted}>Select an employee to view balances.</Text></Box> : loading ? <SimpleGrid columns={{ base: 1, md: 3 }} spacing={3}><Skeleton h="130px" /><Skeleton h="130px" /><Skeleton h="130px" /></SimpleGrid> : eligible.length === 0 ? <Box py={12} textAlign="center" borderWidth="1px" borderStyle="dashed" borderColor={borderColor} borderRadius="md"><Text color={muted}>No effective leave policy for this employee.</Text></Box> : <SimpleGrid columns={{ base: 1, md: 2, xl: 3 }} spacing={3}>{eligible.map((item) => <Box key={item.leaveType._id} borderWidth="1px" borderColor={borderColor} borderRadius="md" p={4}><Flex justify="space-between"><HStack><Box boxSize="10px" borderRadius="sm" bg={item.leaveType.color} /><Text fontWeight="800">{item.leaveType.name}</Text></HStack><HStack><Badge>{item.leaveType.code}</Badge>{item.rule?.entitlementMode === "earned" ? <Badge colorScheme="teal">Earned</Badge> : null}</HStack></Flex><Text mt={4} fontSize="2xl" fontWeight="800">{item.balance.availableUnits || 0}</Text><Text fontSize="xs" color={muted}>Available {item.leaveType.unit}</Text><HStack mt={3} fontSize="xs" color={muted} justify="space-between"><Text>{item.balance.balanceUnits || 0} posted</Text><Text>{item.balance.pendingUnits || 0} pending</Text></HStack>{item.rule?.entitlementMode === "earned" ? <Text mt={3} fontSize="xs" color={muted}>Credits are managed through approved comp-off claims.</Text> : null}{canManage ? <HStack mt={4}>{item.rule?.entitlementMode !== "earned" ? <Button size="xs" colorScheme="blue" leftIcon={<FiEdit2 />} onClick={() => openAdjustment(item)}>Adjust</Button> : null}<Button size="xs" variant="ghost" leftIcon={<FiRefreshCw />} onClick={() => rebuild(item)}>Rebuild</Button></HStack> : null}</Box>)}</SimpleGrid>}
